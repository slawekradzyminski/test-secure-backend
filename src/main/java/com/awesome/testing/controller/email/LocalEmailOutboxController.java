...
@ApiResponse(responseCode = "401", description = "Unauthorized or invalid Bearer token",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorDto.class)))
@ApiResponse(responseCode = "403", description = "Administrator role required, or configured X-Local-Outbox-Key is missing or invalid",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorDto.class)))
...
public class LocalEmailOutboxController {
    ...
    @Operation(summary = "Fetch all queued emails produced in the local profile",
            description = "Returns messages captured by the local email outbox stub. Requires an administrator and, when password-reset.require-outbox-access-key is enabled, X-Local-Outbox-Key. Available only in local and aitesters profiles.")
    ...
    @ApiResponse(responseCode = "200", description = "Outbox buffer cleared successfully", content = @Content)
